...
| Packaging | Multi-arch container (arm64 + amd64), single image; includes ImageMagick (image→PDF), LibreOffice (office→PDF), and Ghostscript (ImageMagick's SVG→PDF delegate shells out to `gs`; SVG is an accepted input) — liteparse shells out to all three. LibreOffice adds ~400MB–1GB to the image; accepted, disk is cheap and a slim/full variant matrix isn't | Matches podman/docker homelab standard | Low |
