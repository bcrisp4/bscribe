...
- **`ocr=force` in v1?** Resolved: dropped (2026-07-06, during M1.2). liteparse has no force-OCR — OCR control is a single boolean `ocr_enabled` (verified in the 2.4.0 Python wrapper and the Rust core `config.rs`); `true` already OCRs every page its complexity detection flags, i.e. our `auto`. So v1 exposes `ocr = auto | off`; re-adding `force` if liteparse grows support is an additive, non-breaking change. Related consequence: liteparse's `ParseResult` reports no `ocr_used` flag, so the adapter derives it — `false` when `ocr=off`, else whether any page's `is_complex()` check said `needs_ocr` — the same signal liteparse's own OCR routing uses.
